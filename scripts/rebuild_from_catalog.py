...
def find_catalog():
    for base in (UPLOADS, REPO.parent / "kaynak-veriler"):
        hits = sorted(glob.glob(str(base / "MHTT_KATALOG*.xlsx")))
        if hits: return hits[0]
    raise FileNotFoundError("MHTT_KATALOG*.xlsx not found")

SHEET_TO_HZ = {
    'Arab Yarımadası':'arabistan','Biladüşşam Havzası':'biladussam','Mısır Havzası':'misir',
    'Mağrib Havzası':'magrib','Endülüs Havzası':'endulus','İran Havzası':'iran','Hint Havzası':'hint',
    'Türkistan Havzası':'turkistan','Balkan':'balkanlar','Balkan Havzası':'balkanlar',
}
...
def main():
    ...
    # ---- parse catalog -> rows per havza ----
    xl = pd.ExcelFile(CAT)
    rows_by_hz = defaultdict(list)
    for sh, hz in SHEET_TO_HZ.items():
        if sh not in xl.sheet_names: continue
        df = xl.parse(sh, header=0)
        cm = resolve_cols(df)
        if cm['meshur'] is None: continue
        for _, r in df.iterrows():
            if g(r, cm['meshur']): rows_by_hz[hz].append((r, cm))
    ...
    # ============================ REPORT ============================
    print("\n================= FAZ 1: KATALOGDAN KURULDU =================")
    print(f"{'havza':10s} {'cat_rows':>9s} {'authors':>8s} {'works':>7s} {'slug_match':>11s}")
    tot_a = tot_w = 0
    for hz in HAVZA_ORDER:
